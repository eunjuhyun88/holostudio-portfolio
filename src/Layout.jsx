        ...
        <div className="min-h-screen bg-white font-sans text-slate-900">
            ...
            <nav 
                className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b ${
                    isScrolled ? 'bg-white/90 backdrop-blur-md border-slate-200 py-3 shadow-sm' : 'bg-white border-transparent py-5'
                }`}
            >
                <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                    {/* Logo */}
                    <Link to="/" className="text-2xl font-bold tracking-tighter hover:opacity-80 transition-opacity z-50">
                        HOLO<span className="text-indigo-600">STUDIO</span>
                    </Link>
                    ...
                    <div className="hidden md:flex items-center gap-8">
                        ...
                        >
                            <button className="flex items-center gap-1 font-medium text-slate-600 hover:text-indigo-600 transition-colors py-2">
                                Businesses
                                ...
                                <div className="bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden p-2">
                                    {businesses.map((biz) => (
                                        <Link 
                                            ...
                                            className="block px-4 py-3 rounded-lg hover:bg-slate-50 text-sm font-medium text-slate-700 hover:text-indigo-600 transition-colors"
                                        >
                                            {biz.name}
                                        </Link>
                                    ))}
                    ...
                    <div className="fixed inset-0 bg-white z-40 pt-24 px-6 md:hidden overflow-y-auto">
                        <div className="flex flex-col gap-6 text-lg">
                            <div className="space-y-4 border-b border-slate-100 pb-6">
                                <p className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-2">Our Businesses</p>
                                {businesses.map((biz) => (
                                    <Link 
                                        key={biz.name}
                            ...
                            <Button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white rounded-full mt-4">
                                Investment Inquiry
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="grid md:grid-cols-4 gap-12 mb-12">
                        <div className="col-span-1 md:col-span-2">
                            <Link to="/" className="text-2xl font-bold tracking-tighter mb-6 block">
                                HOLO<span className="text-indigo-400">STUDIO</span>
                            </Link>
                        ...
                        <div>
                            ...
                            <ul className="space-y-2">
                                {businesses.map(biz => (
                                    <li key={biz.name}>
                                        <Link to={createPageUrl(biz.path.substring(1))} className="text-slate-400 hover:text-white transition-colors">
                                            {biz.name}
                                        </Link>
                                    </li>
                                ))}
                        ...
                        <div>
                            ...
                            <ul className="space-y-2">
                                <li><button onClick={() => scrollToSection('milestones')} className="text-slate-400 hover:text-white transition-colors">Milestones</button></li>
                                <li><button onClick={() => scrollToSection('team')} className="text-slate-400 hover:text-white transition-colors">Team</button></li>
                                <li><Link to={createPageUrl('Contact')} className="text-slate-400 hover:text-white transition-colors">Contact</Link></li>
                            </ul>
                        </div>
                    </div>
