...
export default function Showcase3D() {
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-indigo-500/30">
            ...
            <main className="relative z-10 pt-20 sm:pt-24 md:pt-32 pb-12 sm:pb-16 md:pb-24 px-3 sm:px-4 md:px-8 lg:px-12 max-w-[1600px] mx-auto">
                <div className="text-center mb-8 sm:mb-12 md:mb-16 relative">
                    ...
                    <p className="text-xs sm:text-sm md:text-base lg:text-xl text-neutral-400 max-w-3xl mx-auto leading-relaxed font-mono px-3 sm:px-4 break-keep">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 md:gap-8 min-h-[600px] sm:min-h-[700px] lg:h-[calc(100vh-400px)]">
                        ...
                        <div className="bg-[#0A0A0A]/80 backdrop-blur-xl border border-white/10 rounded-2xl sm:rounded-3xl p-4 sm:p-6 relative overflow-hidden group">
                            ...
                            {!isUploading ? (
                                ...
                                <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-5 relative z-10">
                                    ...
                                    <div className="space-y-1.5 sm:space-y-2">
                                        <Label className="text-[10px] sm:text-xs uppercase tracking-wider text-neutral-500">File (GLTF/GLB/OBJ)</Label>
                                        <div className="relative group/file">
                                            ...
                                            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                                ...
                                                <span className="text-[10px] sm:text-xs text-neutral-400 group-hover/file:text-white transition-colors px-2 text-center">
                                                    {uploadForm.file ? uploadForm.file.name : (language === 'en' ? 'Drop file or click' : '파일 선택')}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div className="flex-1 overflow-y-auto pr-1 sm:pr-2 space-y-2 sm:space-y-3 custom-scrollbar">
                            ...
                            {models.map(model => (
                                <motion.div 
                                    ...
                                    className={`group p-3 sm:p-4 rounded-xl sm:rounded-2xl border transition-all cursor-pointer relative backdrop-blur-sm active:scale-95 ${
                                        ...
                                        <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-1.5 sm:gap-2 text-[9px] sm:text-[10px] font-mono text-neutral-500 uppercase">
                                                    ...
                                                    <span className="hidden sm:inline">{new Date(model.created_date).toLocaleDateString()}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <Button
                                            variant="ghost"
                                            size="icon"
                                            className="h-7 w-7 sm:h-8 sm:w-8 text-neutral-600 hover:text-red-400 sm:opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white/5 flex-shrink-0 ml-2"
                                            onClick={(e) => {
                                                e.stopPropagation();
                    ...
                    <div className="lg:col-span-8 h-full min-h-[400px] sm:min-h-[500px] relative">
                        ...
                        {selectedModel ? (
                            ...
                            >
                                <div className="flex-1 rounded-xl sm:rounded-2xl overflow-hidden bg-black/40 relative group">
                                    ...
                                    <div className="absolute top-2 sm:top-4 left-2 sm:left-4 pointer-events-none">
                                        <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 rounded-full bg-black/50 backdrop-blur-md border border-white/10 text-[10px] sm:text-xs font-mono text-indigo-300">
                                            <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-indigo-500 animate-pulse" />
                                            <span className="hidden sm:inline">LIVE RENDER</span>
                                ...
                                <div className="bg-[#0A0A0A]/80 border border-white/10 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 flex flex-col gap-3 sm:gap-4 backdrop-blur-xl">
                                    <div>
                                        <div className="flex items-center gap-2 sm:gap-3 mb-1 flex-wrap">
                                            <h2 className="text-lg sm:text-xl md:text-2xl font-bold tracking-tight break-all">{selectedModel.name}</h2>
                                        ...
                                        <div className="flex items-center gap-2 sm:gap-4 text-[10px] sm:text-xs md:text-sm text-neutral-400 font-mono flex-wrap">
                                            <span className="truncate max-w-[150px] sm:max-w-none">ID: {selectedModel.id}</span>
                                            <span className="text-neutral-600 hidden sm:inline">|</span>
                                            <span className="hidden sm:inline">SIZE: UNKNOWN</span>
                                        </div>
                                        {selectedModel.description && <p className="text-neutral-400 text-xs sm:text-sm mt-2 max-w-xl break-keep">{selectedModel.description}</p>}
                                    </div>
                                    <div className="flex gap-2 sm:gap-3 flex-wrap">
                                        <a href={selectedModel.file_url} target="_blank" rel="noopener noreferrer" className="flex-1 sm:flex-initial">
                                            <Button variant="outline" className="gap-1.5 sm:gap-2 border-white/10 bg-white/5 hover:bg-white/10 text-white h-9 sm:h-10 md:h-11 px-3 sm:px-4 md:px-6 text-xs sm:text-sm w-full active:scale-95">
                                                <ExternalLink className="w-3 h-3 sm:w-4 sm:h-4" /> 
                                                <span className="hidden sm:inline">Export</span>
                                                <span className="sm:hidden">Export</span>
                                            </Button>
                                        </a>
                                        <Button className="bg-indigo-600 hover:bg-indigo-500 h-9 sm:h-10 md:h-11 px-3 sm:px-4 md:px-6 shadow-[0_0_20px_rgba(79,70,229,0.3)] text-xs sm:text-sm flex-1 sm:flex-initial active:scale-95">
                                            Details
                                        </Button>
                                    </div>
                                </div>
                            ...
                            <div className="h-full rounded-2xl sm:rounded-3xl bg-[#0A0A0A]/40 border border-white/5 flex flex-col items-center justify-center text-neutral-500 gap-4 sm:gap-6 border-dashed relative overflow-hidden">
                                ...
                                <div className="text-center relative z-10 px-4">
                                    ...
                                    <p className="text-[10px] sm:text-xs md:text-sm font-mono text-neutral-600 uppercase tracking-wider sm:tracking-widest break-keep">Select an artifact from registry</p>
                                </div>
                            </div>
                        )}
                    </div>
