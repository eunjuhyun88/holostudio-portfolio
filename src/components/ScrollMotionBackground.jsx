...
export default function ScrollMotionBackground({ theme = 'dark' }) {
    ...
    const opacity1 = useTransform(scrollYProgress, [0, 0.3, 0.7, 1], 
        theme === 'dark' ? [0.7, 0.9, 0.7, 0.5] : [0.3, 0.5, 0.3, 0.15]
    );
    const opacity2 = useTransform(scrollYProgress, [0, 0.4, 0.8, 1], 
        theme === 'dark' ? [0.5, 0.8, 0.6, 0.8] : [0.15, 0.4, 0.2, 0.5]
    );
    ...
    return (
        <div ref={ref} className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
            ...
            <motion.div 
                style={{ 
                    y: y4,
                    opacity: useTransform(scrollYProgress, [0, 0.5, 1], theme === 'dark' ? [0.6, 0.8, 0.6] : [0.25, 0.45, 0.25])
                }}
            ...
            <motion.div
                style={{
                    ...
                    opacity: useTransform(scrollYProgress, [0, 0.3, 0.7, 1], theme === 'dark' ? [0, 0.25, 0.25, 0] : [0, 0.12, 0.12, 0])
                }}
            ...
            <motion.div
                style={{
                    ...
                    opacity: useTransform(scrollYProgress, [0, 0.4, 0.8, 1], theme === 'dark' ? [0, 0.2, 0.2, 0] : [0, 0.1, 0.1, 0])
                }}
                className={`absolute top-[70%] right-[25%] w-24 h-24 border-2 rounded-xl ${
                    theme === 'dark' ? 'border-purple-400/50' : 'border-purple-400/35'
                }`}
            />
            ...
            <motion.div 
                style={{ 
                    ...
                    opacity: useTransform(scrollYProgress, [0, 0.3, 0.7, 1], theme === 'dark' ? [0.1, 0.15, 0.15, 0.1] : [0.06, 0.1, 0.1, 0.06])
                }}
            ...
            <motion.div 
                style={{ 
                    ...
                    opacity: useTransform(scrollYProgress, [0, 0.3, 0.7, 1], theme === 'dark' ? [0.08, 0.12, 0.12, 0.08] : [0.04, 0.07, 0.07, 0.04])
                }}
            ...
            <motion.div
                ...
                className={`absolute left-0 right-0 h-[300px] bg-gradient-to-b from-transparent to-transparent blur-sm ${
                    theme === 'dark' ? 'via-indigo-400/70' : 'via-indigo-500/50'
                }`}
            />
            ...
            {[...Array(8)].map((_, i) => (
                <motion.div
                    ...
                    style={{
                        ...
                        opacity: useTransform(scrollYProgress, [0, 0.5, 1], theme === 'dark' ? [0, 0.4, 0] : [0, 0.2, 0])
                    }}
                    className={`absolute w-1 h-1 rounded-full blur-[0.5px] ${
                        theme === 'dark' ? 'bg-indigo-400/70' : 'bg-indigo-500/50'
                    }`}
                    initial={{
                        top: `${Math.random() * 100}%`,
